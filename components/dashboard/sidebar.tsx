...
export function Sidebar() {
  ...
  return <aside className="sidebar"><div className="brand side-brand"><span className="brand-mark"><Image src="/freitas-loop.png" alt="Freitas Growth Loop" width={38} height={38} priority/></span><span>Growth <b>Loop</b></span></div><div className="workspace"><span className="workspace-avatar">FG</span><span><small>EMPRESA</small><strong>Freitas Growth AI</strong></span></div><nav>{links.map(([label, href, Icon]) => <Link className={path === href ? "active" : ""} href={href} key={href}><Icon size={19}/>{label}</Link>)}</nav><div className="side-user"><span className="avatar">{data?.user?.name?.[0] ?? "U"}</span><span><strong>{data?.user?.name ?? "Usuário"}</strong><small>{data?.user?.email}</small></span><button aria-label="Sair" onClick={() => signOut({ callbackUrl: "/login" })}><LogOut size={18}/></button></div></aside>;
}
